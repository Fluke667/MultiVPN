...
sed -i '/^listen-address.*/d' /usr/local/etc/privoxy/config
echo "listen-address  ${PRV_SERVER}:${PRV_SERVER_PORT}" >> /usr/local/etc/privoxy/config
echo "forward-socks5   /               ${PRV_LOCAL}:${PRV_LOCAL_PORT} ." >> /usr/local/etc/privoxy/config
chown privoxy.privoxy /usr/local/etc/privoxy/*


privoxy --no-daemon --user privoxy /usr/local/etc/privoxy/config &
