...
### Compile Section 3A - Get & Configure & Make Files
    ...
    cd Privoxy-Silent && aclocal && autoheader && autoconf && ./configure --build=Alpine --host=vps01 --prefix=/usr --localstatedir=/var/ --enable-zlib --enable-dynamic-pcre --with-user=privoxy --with-group=privoxy --sysconfdir=/etc/privoxy --docdir=/usr/share/doc/privoxy && make && \
