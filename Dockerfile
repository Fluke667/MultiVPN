...
RUN apk add --update --no-cache alpine-baselayout alpine-base busybox openrc musl musl-dev geoip \
    ...
    libxslt-dev w3m c-ares zlib pcre coreutils libc6-compat libstdc++ lzo && \
    ...
    linux-headers curl openssl-dev zlib-dev git libcork-dev libbloom-dev ipset-dev gcc g++ gmp-dev lzo-dev && \
### PYTHON SECTION
